...
public class MemoryTools
{
	...
	{
		return calculateAvailableSizeInMB(getInternalSDcardPath(context));
	}

	/***********************计算手机外置存储卡剩余大小*********************/
	public static final float getAvailableExternalSDcardSize(Context context)
	{
		return calculateAvailableSizeInMB(getExternalSDcardPath(context));
	}

	/**********************************计算运行内存剩余大小***********************************/
	public static final float getAvailableRomSize()
	{
		return calculateAvailableSizeInMB(getRomPath());
	}
	...
	/*********计算具体内存的可用剩余大小.(以MB计算)********/
	public static final float calculateAvailableSizeInMB(String path)
	{
	...
	public static final float calculateTotalSizeInMB(String path)
	{
		StatFs stat = getStatFs(path);
		if (stat != null)
		{
			return (float)(Math.round((stat.getBlockCount() * (stat.getBlockSize() / (1024f * 1024f)))*100))/100;
		}
		return 0.0f;
	}
